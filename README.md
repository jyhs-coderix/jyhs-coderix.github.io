...
일반 부원은 홈페이지의 `글 작성` 페이지로 글 재료를 작성합니다.
...
1. 홈페이지 상단의 `글 작성`을 누릅니다.
2. 작성하려는 종류를 고릅니다.

```txt
활동 기록: 동아리 활동, 스터디, 행사, 회고
개발 글: 배운 내용, 오류 해결, 기술 정리, 개발 과정
프로젝트: 만든 프로그램, 웹사이트, 앱, 발표 자료, 시연 결과
```

3. 폼을 작성하고 `GitHub에서 작성 계속하기`를 누릅니다.
4. GitHub 이슈 화면이 열리면 사진을 본문 입력칸에 끌어다 놓거나 붙여넣습니다.
5. 업로드가 끝나 이미지 링크가 보이면 `Create`를 누릅니다.
...
이미지 첨부: 사진이 있으면 다음 GitHub 화면의 본문 입력칸에 끌어다 놓거나 붙여넣습니다.
...
아래 템플릿은 `글 작성` 페이지의 `본문` 칸에 그대로 복사해서 사용할 수 있습니다. 괄호 안의 설명만 지우고 자기 내용으로 바꾸면 됩니다.
...
위 3개는 이슈 폼을 고르면 제목 앞에 자동으로 붙습니다. 예: `[개발 글] React 상태 관리 정리`
...
운영진은 검수 후 `초안 생성` 라벨만 붙이면 됩니다. 오른쪽 `Type`은 선택하지 않아도 자동화가 작동합니다.
...
새 프로젝트는 `글 작성` 페이지로 작성하거나, 운영진이 `templates/project.md`를 복사해서 `src/content/projects/` 아래에 넣습니다.
...
이미지는 `public/images/...` 아래에 저장합니다. GitHub 이슈에 첨부한 이미지는 자동 초안 생성 시 글 단위 폴더로 저장됩니다.
